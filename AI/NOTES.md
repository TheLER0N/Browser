...
- ✅ **Реализован обход блокировок**: ProxyService, DoH (Cloudflare/Google), SOCKS5/HTTP прокси, GoodbyeDPI, Xray VPN (vless/vmess ключи)
- Метод: `--proxy-server` аргумент через CoreWebView2EnvironmentOptions для прокси и Xray, аргументы подмены для GoodbyeDPI — НЕ требует установки драйверов вручную (скачиваются автоматически)
- ✅ **Автоприменение**: `ReinitializeEnvironmentAsync()` — пересоздаёт среду WebView2 без перезапуска браузера, URL вкладок сохраняются
- UI Настроек "Сети" был переработан на интуитивные карточки, скрывающие всю сложность.
...
**A:** Для DNS-блокировок — DoH Cloudflare или Google. Для локального обхода ТСПУ — GoodbyeDPI. Для полной подмены IP и обхода глубоких блокировок (vless/vmess) — используйте VPN (Xray) вкладку с ключами. SOCKS5/HTTP тоже поддерживаются в этом же поле.
## Разное
